...
function renderWeatherData(data) {
    const { weatherResponse, daysTemp } = data;
    
    const content = `
        <button class="refresh-btn" onclick="refreshCurrentSearch()">Refresh</button>

        <div class="forecast-section">
            <div class="section-title">${daysTemp.length}-Day Forecast</div>
            <div class="forecast-container">
                ${daysTemp.map((day, index) => {
...
async function loadWeatherData(city, days) {
    try {
        const response = await fetch(`${API_CONFIG.baseUrl}${API_CONFIG.endpoint}?city=${city}&days=${days}`);
        const data = await response.json();

        if (!data.weatherResponse || !data.daysTemp) {
